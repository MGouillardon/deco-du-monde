...
const completeEvent = () => {
  completeForm.post(`/admin/dashboard/events/${props.event.id}/complete`, {
    preserveScroll: true,
  })
}
...
const eventStatus = computed(() => {
  if (props.event.completed) {
    return {
      label: 'Completed',
      class: 'badge-success',
      date: DateTime.fromISO(props.event.completedAt).toFormat('dd/MM/yyyy HH:mm'),
    }
  }
...
<template>
  ...
  <div class="flex justify-between items-center mb-6">
    <div class="flex items-center gap-4">
      ...
      <span v-if="event.completed" class="text-sm text-gray-500">
        Completed on {{ eventStatus.date }}
      </span>
    </div>

    <div v-if="!event.completed" class="flex gap-2">
      <button
        type="button"
      ...
      >
        {{ completeForm.processing ? 'Marking as Complete...' : 'Mark as Complete' }}
      </button>
    </div>
  </div>

  <div class="card bg-base-100 shadow-xl">
    <form @submit.prevent="submit" class="card-body" :class="{ 'opacity-50': event.completed }">
      <fieldset :disabled="event.completed">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="(count, role) in requiredRoles" :key="role" class="form-control w-full">
            ...
            >
              ...
              <option
                v-for="user in usersByRole[role.toLowerCase()]"
                :key="user.id"
                :value="user.id"
              >
                {{ user.fullName }}
